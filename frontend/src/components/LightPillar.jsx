...
const LightPillar = ({
  ...
  useEffect(() => {
    ...
    // Use fixed dimensions to prevent any visual changes when container resizes
    // These values are hardcoded in the shader too - don't change them
    ...
    // Always use fixed size for consistent rendering - never change
    renderer.setSize(fixedWidth, fixedHeight, false); // false = don't update style
    renderer.setPixelRatio(1); // Fixed pixel ratio = 1 to prevent any scaling
    const canvas = renderer.domElement;
    
    // Lock canvas to fixed internal resolution - CSS will scale it visually
    ...
    // Store original setSize to prevent any external changes
    const originalSetSize = renderer.setSize.bind(renderer);
    renderer.setSize = function(w, h, updateStyle) {
      // Always use fixed size internally, ignore any resize requests
      originalSetSize(fixedWidth, fixedHeight, false);
    };
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        uTime: { value: 0 },
        uResolution: { value: new THREE.Vector2(1200.0, 300.0) }, // Hardcoded - never changes
    ...
    const animate = currentTime => {
      if (!isAnimating || !materialRef.current || !rendererRef.current || !sceneRef.current || !cameraRef.current) {
        return;
      }
    ...
    // No resize handling - keep fixed size always to prevent any visual changes
