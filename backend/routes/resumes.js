...
function createResumesRoutes(
  ...
  if (apiKey) {
    ...
    console.log(
      `🔑 [RESUMES] Google API Key loaded: ${keyPreview} (${apiKey.length} chars)`
    );
  } else {
    console.warn(
      "⚠️ [RESUMES] GOOGLE_API_KEY not found in environment variables!"
    );
  }
  ...
  if (openaiApiKey) {
    ...
    console.warn(
      "⚠️ [RESUMES] OPENAI_API_KEY not found - OpenAI fallback will not be available"
    );
  }
  ...
  async function callOpenAI(prompt, temperature = 0.2, userId = null) {
    ...
    console.log(
      "🔄 [OPENAI FALLBACK] Using OpenAI as fallback for Gemini rate limit..."
    );
    ...
    try {
      const completion = await trackApiCall(
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are a helpful assistant that returns valid JSON responses. Always return only valid JSON without markdown formatting.",
              },
              {
                role: "user",
                content: prompt,
          ...
          }),
        {
          ...
          requestPayload: {
            model: "gpt-4o-mini",
            purpose: "resume_processing_fallback",
          },
          estimateCost: 0.001,
        }
      );
      ...
      console.error(
        "❌ [OPENAI FALLBACK] OpenAI API error:",
        openaiError.message
      );
      throw openaiError;
    }
  ...
  async function callGeminiWithRetry(
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        console.log(
          `🤖 [GEMINI] Attempting API call (attempt ${
            retryCount + 1
          }/${maxRetries})...`
        );
        ...
        try {
          ...
          await logApiUsage({
            ...
            tokensUsed,
            success: true,
            costEstimate: tokensUsed
              ? (tokensUsed / 1000000) * 0.000125
        ...
        if (isRateLimit) {
          ...
          if (openai && !usedFallback) {
            console.warn(
              `⚠️ [GEMINI] Rate limit hit (429). Attempting OpenAI fallback...`
            );
            try {
              const temperature =
                modelConfig.generationConfig?.temperature || 0.2;
              const text = await callOpenAI(prompt, temperature, userId);
              usedFallback = true;
              return text;
            } catch (fallbackError) {
              console.error(
                "❌ [OPENAI FALLBACK] Fallback also failed:",
          ...
          if (retryCount < maxRetries) {
            ...
            console.warn(
              `⚠️ [GEMINI] Rate limit hit (429). Waiting ${
                waitTime / 1000
              }s before retry ${retryCount + 1}/${maxRetries}...`
            );
            console.warn(
              `   Error details:`,
              apiError.message?.substring(0, 200)
            );
    ...
    if (openai && !usedFallback && lastError) {
      ...
      if (isRateLimit) {
        console.warn(
          `⚠️ [GEMINI] All retries exhausted. Attempting OpenAI fallback as last resort...`
        );
        try {
          ...
          console.error(
            "❌ [OPENAI FALLBACK] Final fallback attempt failed:",
            fallbackError.message
          );
        }
      }
    ...
    throw (
      lastError ||
      new Error("Failed to get response from Gemini API after retries")
    );
  }
...
`;
      ...
      if (
        ...
        return res.status(429).json({
          ...
          message:
            "You've hit the Gemini API rate limit. Even Tier 1 keys have per-minute limits (typically 60 requests/minute). The system will automatically retry with exponential backoff.",
          details: err.message,
          suggestion:
            "Wait a few seconds and try again, or reduce the frequency of requests.",
          retryAfter:
  ...
  router.get("/", auth, async (req, res) => {
    try {
      ...
      const payload = { resumes: filteredResumes };
  ...
  router.delete("/:id", auth, async (req, res) => {
    try {
      ...
      try {
        ...
        console.log(
          `✅ Cleaned up resume_versions references to deleted resume ${resumeId}`
        );
        ...
        if (!versionCleanupError.message?.includes("published_resume_id")) {
          console.warn(
            "⚠️ Could not clean up resume_versions references:",
            versionCleanupError.message
          );
        }
      }
  ...
  router.get("/:id/download", auth, async (req, res) => {
    try {
      ...
      if (/test/.test(resumeTitleLower)) {
        ...
        console.log(
          `📎 [RESUME DOWNLOAD] Matched "test" pattern for "${resume.title}"`
        );
      }
      // Priority 2: Check if title contains "adit" (case-insensitive)
        ...
        console.log(
          `📎 [RESUME DOWNLOAD] Matched "adit" pattern for "${resume.title}"`
        );
      }
      ...
      if (hardcodedFile && requestedFormat) {
        ...
        if (finalHardcodedPath) {
          console.log(
            `✅ [RESUME DOWNLOAD] Serving hardcoded file: ${hardcodedFile} for "${resume.title}"`
          );
          const contentTypes = {
          ...
          res.setHeader(
            "Content-Type",
            contentTypes[requestedFormat] || "application/octet-stream"
          );
          res.setHeader(
            "Content-Disposition",
            `attachment; filename="${resume.title}.${requestedFormat}"`
          );
          return res.sendFile(path.resolve(finalHardcodedPath));
        } else {
          console.warn(
            `⚠️ [RESUME DOWNLOAD] Hardcoded file not found: ${hardcodedFile} (tried: ${hardcodedPath})`
          );
      ...
      if (resume.file_url) {
        ...
        if (finalPath) {
          ...
          if (
            ...
            try {
              console.log(
                `🔄 [RESUME DOWNLOAD] Converting ${ext} to PDF for viewing`
              );
              ...
              const tempHtmlPath = path.join(
                tempDir,
                `resume-${resume.id}-${Date.now()}.html`
              );
              const tempPdfPath = path.join(
                tempDir,
                `resume-${resume.id}-${Date.now()}.pdf`
              );
            ...
            `;
              ...
              res.setHeader(
                "Content-Disposition",
                `inline; filename="${resume.title}.pdf"`
              );
              ...
              console.log(
                `✅ [RESUME DOWNLOAD] Converted and served PDF: ${tempPdfPath}`
              );
              return;
            } catch (convErr) {
          ...
          if (
            (ext === ".pdf" || ext === ".txt") &&
            (!requestedFormat || requestedFormat === ext.replace(".", ""))
          ) {
            const contentTypes = {
              ".pdf": "application/pdf",
              ".txt": "text/plain",
          ...
          if (requestedFormat && requestedFormat !== ext.replace(".", "")) {
            console.log(
              `🔄 [RESUME DOWNLOAD] Format conversion requested: ${ext} -> ${requestedFormat}`
            );
            ...
            try {
              ...
              if (extractedText && extractedText.trim()) {
                console.log(
                  `✅ [RESUME DOWNLOAD] Extracted ${extractedText.length} characters from uploaded file`
                );
                ...
                console.log(
                  `📄 [RESUME DOWNLOAD] Created sections from extracted text`
                );
              } else {
                console.warn(
                  `⚠️ [RESUME DOWNLOAD] Could not extract text from uploaded file for format conversion`
                );
              }
            } catch (extractErr) {
              console.error(
                "❌ [RESUME DOWNLOAD] Error extracting text:",
                extractErr
              );
            }
            ...
            console.warn(
              `⚠️ [RESUME DOWNLOAD] Format conversion requested but no sections available for uploaded file`
            );
          }
        } else {
          console.warn(
            `⚠️ Uploaded file not found: ${resume.file_url} (tried: ${filePath})`
          );
      ...
      if (!sections) {
        try {
          if (typeof resume.sections === "string") {
            ...
            console.warn(
              "⚠️ [RESUME DOWNLOAD] No sections found in resume, creating empty structure"
            );
            sections = normalizeSections({});
          }
        } catch (parseErr) {
      ...
      console.log("📄 [RESUME DOWNLOAD] Sections data:", {
        ...
        rawSectionsLength:
          typeof resume.sections === "string" ? resume.sections.length : "N/A",
      });

      // Allow format to be specified via query parameter, otherwise use resume's format
      const format =
        requestedFormat && ["pdf", "docx", "txt"].includes(requestedFormat)
          ? requestedFormat
...
<body>`;
        ...
        if (sections.summary?.contact) {
          ...
          if (contactParts.length > 0) {
            htmlContent += `<div class="contact">${contactParts.join(
              " | "
            )}</div>`;
          }
        }
        ...
        if (sections.summary?.title) {
          htmlContent += `<div class="title">${escapeHtml(
        ...
        if (sections.summary?.bio) {
          ...
          <div style="font-size: 10pt; margin-bottom: 12px;">${escapeHtml(
        ...
        if (
          ...
          sections.experience.forEach((exp) => {
            ...
            if (exp.title || exp.role)
              titleCompany.push(escapeHtml(exp.title || exp.role));
            if (exp.company) titleCompany.push(escapeHtml(exp.company));

            if (titleCompany.length > 0) {
              htmlContent += `<div class="job-header">${titleCompany.join(
                " - "
              )}</div>`;
            }
            ...
            if (exp.start_date) {
              ...
              metaInfo.push(
                `${escapeHtml(exp.start_date)} - ${escapeHtml(endDate)}`
              );
            }
            if (metaInfo.length > 0) {
              htmlContent += `<div class="job-meta">${metaInfo.join(
                " | "
              )}</div>`;
            }
              ...
              bullets.forEach((bullet) => {
                const bulletText =
                  typeof bullet === "string"
                    ...
                    : bullet.text || "";
                if (bulletText) {
                  const cleanText = bulletText.startsWith("•")
                    ? bulletText.replace(/^•\s*/, "")
                    : bulletText;
                  htmlContent += `<div class="bullet-item">• ${escapeHtml(
                    cleanText
                  )}</div>`;
                }
              });
        ...
        if (
          ...
          sections.education.forEach((edu) => {
            ...
            if (edu.degree_type && edu.field_of_study) {
              eduParts.push(
                `${escapeHtml(edu.degree_type)} in ${escapeHtml(
                  edu.field_of_study
                )}`
              );
            } else if (edu.education_level && edu.field_of_study) {
              eduParts.push(
                `${escapeHtml(edu.education_level)} in ${escapeHtml(
                  edu.field_of_study
                )}`
              );
            }
            ...
            if (edu.graduation_date)
              eduParts.push(escapeHtml(edu.graduation_date));
            if (edu.gpa) eduParts.push(`GPA: ${escapeHtml(edu.gpa)}`);

            if (eduParts.length > 0) {
              htmlContent += `<div class="education-item">${eduParts.join(
                " | "
              )}</div>`;
            }
          });
        ...
        if (
          ...
          sections.projects.forEach((proj) => {
            if (proj.name) {
              htmlContent += `<div class="project-name">${escapeHtml(
                proj.name
              )}</div>`;
            }
            if (proj.description) {
      ...
      if (format === "txt") {
        ...
        console.log(
          `📄 [RESUME DOWNLOAD] Creating TXT with ${textContent.length} characters`
        );
...
`;
      ...
      const normalized = {
        summary: reconciled.summary || {
          ...
          bio:
            reconciled.summary_recommendation ||
        ...
        experience: expSource.map((e) => ({
          ...
          end_date:
            e.end_date || masterResume.experience?.[0]?.end_date || null,
          current:
