...
export function createQualityScoringService(
  ...
  async function callOpenAIWithRetry(messages, maxRetries = 3) {
    ...
    if (hasImages) {
      console.log(
        `🖼️ [QUALITY SCORING] Using vision model (${model}) for PDF analysis`
      );
    }
    ...
    while (retryCount < maxRetries) {
      try {
        console.log(
          `🤖 [QUALITY SCORING] Attempting OpenAI API call (attempt ${
            retryCount + 1
          }/${maxRetries})...`
        );
  ...
  /**
   * Extract text from PDF buffer using pdf-parse (pure JS, no native binaries)
   */
  ...
  /**
   * Convert PDF pages to base64 images for vision API using puppeteer
   * Note: Uses pdf-parse for page count, and CDN pdf.js in browser for rendering
   */
  async function convertPdfToImages(filePath) {
    ...
    try {
      ...
      // Get PDF page count using pdf-parse (pure JS, no native binaries)
      ...
      // Create HTML page with CDN pdf.js to render PDF (browser-side, not Node.js pdfjs-dist)
      const htmlContent = `
<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    pdfjsLib.getDocument(pdfDataUri).promise.then(function(pdf) {
      ...
      for (let i = 1; i <= numPages; i++) {
        try {
          ...
          const pageHtml = htmlContent.replace(
            "const pageNum = parseInt(new URLSearchParams(window.location.search).get('page') || '1');",
            `const pageNum = ${i};`
          );
          ...
          console.log(
            `✅ [QUALITY SCORING] Converted page ${i}/${numPages} to image`
          );
        } catch (pageErr) {
          console.error(
            `❌ [QUALITY SCORING] Error converting page ${i}:`,
            pageErr.message
          );
      ...
      console.log(
        `✅ [QUALITY SCORING] Converted ${images.length} PDF page(s) to images`
      );

      return images;
    } catch (err) {
      console.error(
        "❌ [QUALITY SCORING] Error converting PDF to images:",
        err
      );
      if (browser) {
        try {
  ...
  async function aggregateApplicationMaterials(jobId, userId) {
    try {
      console.log(
        `📋 [QUALITY SCORING] Fetching job ${jobId} for user ${userId}`
      );
      ...
      console.log(
        `✅ [QUALITY SCORING] Job found: ${job.title} at ${job.company}`
      );
      ...
      if (materialsResult.rows.length > 0) {
        ...
        if (jobMaterialsResult.rows.length > 0) {
          ...
          console.log(
            `⚠️ [QUALITY SCORING] Materials found in jobs table (not in job_materials). Consider migrating.`
          );
        } else {
          console.warn(
            `⚠️ [QUALITY SCORING] No materials found for job ${jobId}. Resume and cover letter may not be linked.`
          );
        }
      }
        ...
        if (resumeResult.rows.length > 0) {
          ...
          if (resume.sections) {
            try {
              ...
              console.warn(
                `⚠️ [QUALITY SCORING] Failed to parse resume sections for resume ${resume.id}:`,
                parseErr.message
              );
              parsedSections = null;
            }
            ...
            if (fs.existsSync(filePath)) {
              try {
                ...
                if (resume.format === "pdf") {
                  ...
                  console.log(
                    `📄 [QUALITY SCORING] Extracted ${resumeData.text.length} characters from PDF`
                  );
                } else if (
                  resume.format === "docx" ||
                  resume.format === "doc"
                  ...
                  console.log(
                    `📄 [QUALITY SCORING] Extracted ${resumeData.text.length} characters from DOCX`
                  );
                } else if (resume.format === "txt") {
                  resumeData.text = buffer.toString("utf-8");
                  console.log(
                    `📄 [QUALITY SCORING] Extracted ${resumeData.text.length} characters from TXT`
                  );
                }
              } catch (fileErr) {
                console.error(
                  `❌ [QUALITY SCORING] Error extracting text from file:`,
                  fileErr
                );
              }
            } else {
              console.warn(
                `⚠️ [QUALITY SCORING] Resume file not found at: ${filePath}`
              );
            }
          }
          ...
          if (parsedSections) {
            console.log(`📋 [QUALITY SCORING] Resume sections structure:`, {
              ...
              skillsType: parsedSections.skills
                ? typeof parsedSections.skills
                : "none",
              skillsIsArray: Array.isArray(parsedSections.skills),
              skillsValue: parsedSections.skills
                ? Array.isArray(parsedSections.skills)
                  ? parsedSections.skills.slice(0, 5)
                  : typeof parsedSections.skills === "object"
                  ? Object.keys(parsedSections.skills).slice(0, 5)
                  : String(parsedSections.skills).substring(0, 100)
                : "none",
              hasExperience: !!parsedSections.experience,
              hasEmployment: !!parsedSections.employment,
              experienceType: parsedSections.experience
                ? typeof parsedSections.experience
                : "none",
              experienceIsArray: Array.isArray(parsedSections.experience),
              experienceCount: parsedSections.experience
                ? Array.isArray(parsedSections.experience)
                  ? parsedSections.experience.length
                  : typeof parsedSections.experience === "object"
                  ? Object.keys(parsedSections.experience).length
                  : 0
                : 0,
            });
          }
      ...
      if (materials.cover_letter_id) {
        console.log(
          `✉️ [QUALITY SCORING] Fetching cover letter ${materials.cover_letter_id} for job ${jobId}`
        );
        ...
        if (coverLetterResult.rows.length > 0) {
          ...
          if (!coverLetterData.text && coverLetter.file_url) {
            try {
              ...
              console.log(
                `✉️ [QUALITY SCORING] Attempting to read cover letter file: ${filePath}`
              );
              if (fs.existsSync(filePath)) {
                const buffer = fs.readFileSync(filePath);
                if (coverLetter.format === "pdf") {
                  ...
                  coverLetter.format === "docx" ||
                  coverLetter.format === "doc"
                ) {
                ...
                console.log(
                  `✅ [QUALITY SCORING] Extracted ${
                    coverLetterData.text?.length || 0
                  } characters from cover letter file`
                );
              } else {
                console.warn(
                  `⚠️ [QUALITY SCORING] Cover letter file not found at: ${filePath}`
                );
              }
            } catch (fileErr) {
              console.error(
                `❌ [QUALITY SCORING] Error reading cover letter file:`,
                fileErr
              );
      ...
      if (!resumeData && !coverLetterData) {
        console.warn(
          `⚠️ [QUALITY SCORING] No application materials found for job ${jobId}. Analysis may be incomplete.`
        );
      } else {
        console.log(
          `✅ [QUALITY SCORING] Materials found: Resume=${
            resumeData ? `Yes (ID: ${resumeData.id})` : "No"
          }, Cover Letter=${
            coverLetterData ? `Yes (ID: ${coverLetterData.id})` : "No"
          }`
        );
      }
      ...
      console.error(
        "❌ [QUALITY SCORING] Error aggregating application materials:",
        err
      );
  ...
  async function generateQualityScoreMessages(materials, jobDescription) {
    ...
    if (resume?.file_url && resume?.format === "pdf") {
      try {
        ...
        if (fs.existsSync(filePath)) {
          console.log(
            `🖼️ [QUALITY SCORING] Converting resume PDF to images for vision analysis`
          );
          ...
          if (!resume.text || resume.text.trim().length < 50) {
            ...
            if (extractedText && extractedText.trim().length > 50) {
              ...
              console.log(
                `📄 [QUALITY SCORING] Extracted PDF text and rebuilt resume text (${resumeText.length} chars)`
              );
            }
          }
        }
    ...
    if (!resumeText || resumeText.trim().length < 50) {
      if (resume?.sections) {
        ...
        console.log(
          `📄 [QUALITY SCORING] Using sections JSON as resume text (fallback)`
        );
      }
    }
    ...
    console.log(
      `📄 [QUALITY SCORING] Final resume text length: ${resumeText.length} characters`
    );
    if (resumeText.length < 200) {
      console.warn(
        `⚠️ [QUALITY SCORING] Resume text is very short (${resumeText.length} chars). AI may not have enough content to analyze.`
      );
    }

    const coverLetterText =
      coverLetter?.content || coverLetter?.text || "Cover letter not provided";
    ...
    if (resume) {
      ...
      console.log(
        `🔍 [QUALITY SCORING] Extracted ${skillCount} skills from resume: ${allSkills
          .slice(0, 15)
          .join(", ")}${allSkills.length > 15 ? "..." : ""}`
      );

      // If no skills found from sections, try extracting from text
      if (skillCount === 0 && resume.text) {
        console.log(
          `⚠️ [QUALITY SCORING] No skills found in sections, attempting text extraction...`
        );
        ...
        if (textSkills.length > 0) {
          ...
          console.log(
            `✅ [QUALITY SCORING] Extracted ${skillCount} skills from text: ${textSkills
              .slice(0, 15)
              .join(", ")}${textSkills.length > 15 ? "..." : ""}`
          );
        }
      }
      ...
      const experienceSection =
        resume.sections?.experience || resume.sections?.employment;
      ...
      if (experienceSection) {
        if (Array.isArray(experienceSection)) {
            ...
            if (exp && typeof exp === "object") {
              ...
              if (exp.description) {
                  ...
                  const actualBullets = lines.filter((line) => {
                    ...
                    return (
                      ...
                      (line.length > 10 &&
                        /^[A-Z]/.test(line) &&
                        line.match(
                          /\b(?:built|developed|created|designed|implemented|managed|led|improved|increased|reduced|optimized|achieved|delivered)\b/i
                        ))
                    ); // Action verb bullets
                  });
                  bulletsFromDescription = actualBullets.length;
                  actualBullets.forEach((b) =>
                    countedBullets.add(b.toLowerCase().trim())
                  );
                } else if (Array.isArray(exp.description)) {
                  bulletsFromDescription = exp.description.filter(
                    (b) => b && b.trim().length > 0
                  ).length;
                  exp.description.forEach((b) =>
                    countedBullets.add(String(b).toLowerCase().trim())
                  );
                }
                ...
                console.log(
                  `  Experience ${idx + 1}: ${
                    ...
                    exp.company || "Unknown"
                  } - ${bulletsFromDescription} bullets from description`
                );
              }
              ...
              if (exp.bullets && Array.isArray(exp.bullets)) {
                ...
                if (newBullets.length > 0) {
                  console.log(
                    `  Experience ${idx + 1}: Additional ${
                      newBullets.length
                    } bullets from bullets array (${
                      exp.bullets.length
                    } total, ${
                      exp.bullets.length - newBullets.length
                    } duplicates skipped)`
                  );
                }
              }
              ...
              if (exp.achievements && Array.isArray(exp.achievements)) {
                ...
                if (newAchievements.length > 0) {
                  console.log(
                    ...
                    } bullets from achievements array (${
                      exp.achievements.length
                    } total, ${
                      exp.achievements.length - newAchievements.length
                    } duplicates skipped)`
                  );
                }
              }
          ...
          typeof experienceSection === "object" &&
          !Array.isArray(experienceSection)
        ) {
          // Handle object format where keys might be job IDs or indices
          const expArray = Object.values(experienceSection).filter(
            ...
            if (exp.bullets && Array.isArray(exp.bullets)) {
              ...
              if (newBullets.length > 0) {
                console.log(
                  `  Experience ${idx + 1}: Additional ${
                    newBullets.length
                  } bullets from bullets array (${exp.bullets.length} total, ${
                    exp.bullets.length - newBullets.length
                  } duplicates skipped)`
                );
              }
      ...
      if (experienceCount === 0 && resume.text) {
        console.log(
          `⚠️ [QUALITY SCORING] No experiences found in sections, attempting text extraction...`
        );
        ...
        if (textExperiences.length > 0) {
          ...
          totalBullets = textExperiences.reduce(
            (sum, exp) => sum + (exp.bullets || 0),
            0
          );
          console.log(
            `✅ [QUALITY SCORING] Extracted ${experienceCount} experiences from text with ${totalBullets} total bullets`
          );
        }
      }
        ...
        if (Array.isArray(projectsSection)) {
          ...
          projectsSection.forEach((proj, idx) => {
            if (proj && typeof proj === "object") {
              console.log(
                `  Project ${idx + 1}: ${proj.name || proj.title || "Untitled"}`
              );
            }
          });
        } else if (
          typeof projectsSection === "object" &&
          !Array.isArray(projectsSection)
        ) {
          const projArray = Object.values(projectsSection).filter(
            (proj) => proj && typeof proj === "object"
          );
      ...
      if (projectCount === 0 && resume.text) {
        console.log(
          `⚠️ [QUALITY SCORING] No projects found in sections, attempting text extraction...`
        );
          ...
          if (matches && matches[1]) {
            ...
            for (let i = 0; i < lines.length; i++) {
              ...
              const looksLikeProjectName =
                /^[A-Z][A-Za-z0-9\s\-&]+(?:\s*[:\-]|\s*$)/.test(line) &&
                line.length < 80 &&
                !line.match(
                  /^(?:built|developed|created|designed|implemented|technologies?|tech|stack|tools?):/i
                );

              // Detect if line mentions technologies (common tech keywords)
              const hasTechnologies =
                /\b(?:Python|Java|JavaScript|React|Node\.?js|SQL|AWS|Docker|MongoDB|PostgreSQL|Git|HTML|CSS|TypeScript|Angular|Vue|Flask|Django|Spring|TensorFlow|Pandas|NumPy|Spark|Hadoop)\b/i.test(
                  line
                );

              // Detect project action words
              const hasProjectActions =
                /\b(?:built|developed|created|designed|implemented|deployed|architected|engineered)\b/i.test(
                  line
                );

              if (
                looksLikeProjectName ||
                (hasTechnologies && hasProjectActions)
              ) {
            ...
            while ((bulletMatch = bulletPattern.exec(projectText)) !== null) {
              ...
              if (
                /\b(?:built|developed|created|designed|implemented|Python|Java|JavaScript|React|Node|SQL|AWS|Docker)\b/i.test(
                  bulletText
                )
              ) {
            ...
            if (bullets.length > 0 && foundProjects.length === 0) {
              const estimatedFromBullets = Math.max(
                1,
                Math.floor(bullets.length / 2.5)
              );
              foundProjects = Array(estimatedFromBullets)
                .fill(null)
                .map(() => ({
                  name: "Project",
                  hasDescription: true,
        ...
        if (foundProjects.length > 0) {
          ...
          console.log(
            `✅ [QUALITY SCORING] Extracted ${projectCount} projects from text: ${foundProjects
              .map((p) => p.name)
              .join(", ")}`
          );
        }
      ...
      console.log(
        `📊 [QUALITY SCORING] Resume analysis complete: ${skillCount} skills, ${experienceCount} experiences, ${totalBullets} total bullets, ${projectCount} projects`
      );
    }
...
- **MANDATORY CALCULATION**: 
  * Count required skills from job description: ${
    job.required_skills ? job.required_skills.length : "unknown"
  }
...
- Return ONLY valid JSON, no markdown formatting or additional text`;
    ...
    if (resumeImages.length > 0) {
      console.log(
        `🖼️ [QUALITY SCORING] Adding ${resumeImages.length} resume image(s) to vision API`
      );
    ...
    return [
      { role: "system", content: systemPrompt },
      {
        role: "user",
        content:
          userMessageContent.length === 1 ? userPrompt : userMessageContent,
      },
    ];
  }
  ...
  function extractSkillsFromResume(resume) {
    ...
    if (resume.sections?.projects && Array.isArray(resume.sections.projects)) {
      resume.sections.projects.forEach((proj) => {
        ...
        if (proj && proj.description) {
          ...
          const techPattern =
            /\b(?:Python|JavaScript|React|Node\.?js|SQL|AWS|Docker|Kubernetes|Git|Java|TypeScript|Angular|Vue|MongoDB|PostgreSQL|Redis|GraphQL|REST|API)\b/gi;
    ...
    if (resume.text) {
      ...
      skillPatterns.forEach((pattern) => {
        try {
          const matches = resume.text.matchAll(
  ...
  function extractSkillsFromText(text) {
    ...
    commonSkills.forEach((skill) => {
      ...
      const pattern = skill.includes(".")
        ? new RegExp(
            `\\b${escapedSkill}\\b|${escapedSkill.replace(/\./g, "\\.")}`,
            "i"
          )
  ...
  function extractExperiencesFromText(text) {
    if (!text) {
      console.log(
        `⚠️ [QUALITY SCORING] extractExperiencesFromText: No text provided`
      );
      return [];
    }

    console.log(
      `🔍 [QUALITY SCORING] extractExperiencesFromText: Analyzing ${text.length} characters of text`
    );
    const experiences = [];
    ...
    const companyPositionPattern =
      /([A-Z][A-Za-z0-9\s&,\.\-]+?)\s*\|\s*([A-Z][A-Za-z\s&,\.\-]+)/g;
    ...
    console.log(
      `  Found ${companyMatches.length} "Company | Position" patterns`
    );
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        inExperienceSection &&
        /^(education|summary|technical|projects|skills|certifications|awards|honors)/i.test(
          line
        )
      ) {
        ...
        if (currentExp) {
          ...
          console.log(
            `  Completed experience: ${currentExp.company} | ${currentExp.title} (${currentExp.bullets} bullets)`
          );
          currentExp = null;
        }
      ...
      const companyMatch = line.match(
        /([A-Z][A-Za-z0-9\s&,\.\-]+?)\s*\|\s*([A-Z][A-Za-z\s&,\.\-]+)/
      );
      if (companyMatch) {
        ...
        if (inExperienceSection || i > experienceSectionStart) {
          ...
          console.log(
            `  Found experience at line ${i + 1}: ${currentExp.company} | ${
              currentExp.title
            }`
          );
        }
      ...
      if (datePattern.test(line)) {
        ...
        if (
          !currentExp &&
          (inExperienceSection || i > experienceSectionStart)
        ) {
          // Look back up to 3 lines for company | position pattern
          for (let j = Math.max(0, i - 3); j < i; j++) {
            const prevLine = lines[j].trim();
            const prevCompanyMatch = prevLine.match(
              /([A-Z][A-Za-z0-9\s&,\.\-]+?)\s*\|\s*([A-Z][A-Za-z\s&,\.\-]+)/
            );
            if (prevCompanyMatch) {
              currentExp = {
              ...
              console.log(
                `  Found experience from date line ${i + 1}: ${
                  currentExp.company
                } | ${currentExp.title}`
              );
              break;
    ...
    if (currentExp) {
      experiences.push(currentExp);
      console.log(
        `  Completed final experience: ${currentExp.company} | ${currentExp.title} (${currentExp.bullets} bullets)`
      );
    }
    ...
    if (experiences.length === 0 && companyMatches.length > 0) {
      console.log(
        `  No structured experiences found, creating from ${companyMatches.length} company|position patterns`
      );
      companyMatches.forEach((match, idx) => {
        // Estimate bullets: count bullets between this match and the next match (or end)
        const nextMatchIndex =
          idx < companyMatches.length - 1
            ? companyMatches[idx + 1].index
            : text.length;
        ...
        console.log(
          `  Created experience: ${match.company} | ${match.title} (${sectionBullets} bullets)`
        );
      });
    }
    ...
    if (experiences.length === 0 && bullets.length > 0) {
      console.log(
        `  Still no experiences found, estimating from ${bullets.length} bullet points`
      );
      ...
      console.log(
        `  Estimated ${estimatedExperiences} experiences with ${bulletsPerExp} bullets each`
      );
    }

    console.log(
      `✅ [QUALITY SCORING] extractExperiencesFromText: Found ${
        experiences.length
      } experiences with ${experiences.reduce(
        (sum, exp) => sum + (exp.bullets || 0),
        0
      )} total bullets`
    );
  ...
  function getResumeText(resume) {
    ...
    let text =
      resume.text && resume.text.trim().length > 50 ? resume.text.trim() : "";

    // Build comprehensive text from sections (to supplement or replace if no file text)
    let sectionsText = "";
    if (resume.sections) {
      if (resume.sections.summary?.bio)
        sectionsText += resume.sections.summary.bio + " ";
      if (resume.sections.summary?.title)
        sectionsText += resume.sections.summary.title + " ";
      if (resume.sections.summary?.full_name)
        sectionsText += resume.sections.summary.full_name + " ";
      ...
      const experienceSection =
        resume.sections.experience || resume.sections.employment;
      if (experienceSection) {
        const expArray = Array.isArray(experienceSection)
          ? experienceSection
          : Object.values(experienceSection);
        expArray.forEach((exp, idx) => {
          if (exp && typeof exp === "object") {
      ...
      if (resume.sections.projects) {
        const projArray = Array.isArray(resume.sections.projects)
          ? resume.sections.projects
          : Object.values(resume.sections.projects);
      ...
      if (resume.sections.education) {
        const eduArray = Array.isArray(resume.sections.education)
          ? resume.sections.education
          : Object.values(resume.sections.education);
    ...
    if (text && sectionsText) {
      ...
      console.log(
        `📋 [QUALITY SCORING] getResumeText: Combined ${text.length} chars from file + ${sectionsText.length} chars from sections = ${combined.length} total`
      );
      return combined;
    } else if (text) {
      console.log(
        `📋 [QUALITY SCORING] getResumeText: Using extracted file text (${text.length} chars)`
      );
      return text;
    } else if (sectionsText) {
      console.log(
        `📋 [QUALITY SCORING] getResumeText: Using sections text (${sectionsText.length} chars)`
      );
  ...
  function cleanMissingKeywordsAndSkills(keywords, skills) {
    ...
    const finalSkills = cleanedSkills.filter((skill) => {
      ...
      const isInKeyword = normalizedKeywords.some(
        (kw) => kw.includes(normalizedSkill) || normalizedSkill.includes(kw)
      );
      return !isInKeyword;
    });
  ...
  function calculateOverallScore(aiResponse) {
    try {
      ...
      const hasLinkedIn =
        data.linkedin_score !== null && data.linkedin_score !== undefined;
      ...
      if (
        data.cover_letter_score !== null &&
        data.cover_letter_score !== undefined
      ) {
   ...
   */
  async function analyzeApplicationQuality(
    jobId,
    userId,
    minimumThreshold = 70
  ) {
    try {
      console.log(
        `🔍 [QUALITY SCORING] Starting analysis for job ${jobId}, user ${userId}`
      );
      ...
      if (!materials.resume && !materials.coverLetter) {
        const errorMsg =
          "No application materials found. Please link a resume or cover letter to this job in the Application Materials section.";
        console.error(`❌ [QUALITY SCORING] ${errorMsg}`);
        throw new Error(errorMsg);
      }

      console.log(
        `✅ [QUALITY SCORING] Materials validated: Resume=${
          materials.resume ? "Yes" : "No"
      ...
      const messages = await generateQualityScoreMessages(
        materials,
        materials.job.description
      );
      ...
      try {
        ...
        console.error(
          "❌ [QUALITY SCORING] Failed to parse AI response:",
          parseErr
        );
      ...
      console.log(
        `📊 [QUALITY SCORING] Raw AI scores - Resume: ${scoreData.resume_score}, Cover Letter: ${scoreData.cover_letter_score}, LinkedIn: ${scoreData.linkedin_score}`
      );
      ...
      if (scoreData.missing_keywords || scoreData.missing_skills) {
        ...
        console.log(
          `✅ [QUALITY SCORING] Cleaned missing items - Keywords: ${processed.keywords.length}, Skills: ${processed.skills.length}`
        );
      }
      ...
      if (scoreData.missing_skills && materials.resume) {
        ...
        console.log(
          `🔍 [QUALITY SCORING] Validating ${scoreData.missing_skills.length} missing skills`
        );
        console.log(
          `📋 [QUALITY SCORING] Extracted ${
            actualSkills.length
          } skills from resume: ${actualSkills.slice(0, 15).join(", ")}${
            actualSkills.length > 15 ? "..." : ""
          }`
        );

        const initialCount = scoreData.missing_skills.length;
        scoreData.missing_skills = scoreData.missing_skills.filter(
          (missingSkill) => {
            ...
            const foundInSkills = actualSkills.some((actualSkill) => {
              ...
              return (
                normalizedMissing === normalizedActual ||
            ...
            const foundVariation = skillVariations.some((variation) => {
              return (
                ...
                actualSkills.some((s) => {
                  ...
                  return (
                    normalizedS.includes(variation) ||
                    variation.includes(normalizedS)
                  );
                })
              );
            });
            ...
            if (found) {
              console.log(
                `✅ [QUALITY SCORING] Removed false positive missing skill: "${missingSkill}"`
              );
            } else {
              console.log(
                `⚠️ [QUALITY SCORING] Confirmed missing skill: "${missingSkill}" (not found in resume)`
              );
            }
        ...
        console.log(
          `✅ [QUALITY SCORING] Validated missing skills - Removed ${removedCount} false positives, ${scoreData.missing_skills.length} remain`
        );
      }
      ...
      if (scoreData.missing_keywords && materials.resume) {
        ...
        console.log(
          `🔍 [QUALITY SCORING] Validating ${initialCount} missing keywords`
        );

        scoreData.missing_keywords = scoreData.missing_keywords.filter(
          (missingKeyword) => {
            ...
            const found =
              resumeText.includes(normalizedMissing) ||
              normalizedMissing.split(" ").every((word) => {
                // Check if all words in the keyword phrase appear in resume (even if not together)
                return (
                  word.length > 2 && resumeText.includes(word.toLowerCase())
                );
              });

            if (found) {
              console.log(
                `✅ [QUALITY SCORING] Removed false positive missing keyword: "${missingKeyword}"`
              );
            } else {
              console.log(
                `⚠️ [QUALITY SCORING] Confirmed missing keyword: "${missingKeyword}"`
              );
            }
        ...
        console.log(
          `✅ [QUALITY SCORING] Validated missing keywords - Removed ${removedCount} false positives, ${scoreData.missing_keywords.length} remain`
        );
      }
      ...
      if (scoreData.score_breakdown) {
        ...
        if (
          materials.resume &&
          (skillsAlignmentScore === 0 ||
            skillsAlignmentScore === null ||
            skillsAlignmentScore === undefined)
        ) {
          const actualSkills = extractSkillsFromResume(materials.resume);
          if (actualSkills.length > 0) {
            console.log(
              `⚠️ [QUALITY SCORING] Fixing skills_alignment score: was ${skillsAlignmentScore}, but resume has ${actualSkills.length} skills. Setting to minimum 30.`
            );
        ...
        const experienceRelevanceScore =
          scoreData.score_breakdown.experience_relevance;
        if (
          materials.resume &&
          (experienceRelevanceScore === 0 ||
            experienceRelevanceScore === null ||
            experienceRelevanceScore === undefined)
        ) {
          const hasExperience =
            materials.resume.sections?.experience &&
            Array.isArray(materials.resume.sections.experience) &&
            materials.resume.sections.experience.length > 0;
          if (hasExperience) {
            console.log(
              `⚠️ [QUALITY SCORING] Fixing experience_relevance score: was ${experienceRelevanceScore}, but resume has ${materials.resume.sections.experience.length} work experience(s). Setting to minimum 30.`
            );
        ...
        Object.keys(scoreData.score_breakdown).forEach((key) => {
          ...
          if (score !== null && score !== undefined) {
            scoreData.score_breakdown[key] = Math.max(
              0,
      ...
      const hasLinkedIn =
        scoreData.linkedin_score !== null &&
        scoreData.linkedin_score !== undefined;

      if (
        scoreData.resume_score !== null &&
        scoreData.resume_score !== undefined
      ) {
        const resumeContribution = scoreData.resume_score * 0.6;
        overallScore += resumeContribution;
        totalWeight += 0.6;
        console.log(
          `  Resume: ${scoreData.resume_score} × 0.6 = ${resumeContribution}`
        );
      }

      if (
        scoreData.cover_letter_score !== null &&
        scoreData.cover_letter_score !== undefined
      ) {
        // If LinkedIn is available, cover letter is 30%, otherwise 40%
        const coverWeight = hasLinkedIn ? 0.3 : 0.4;
        ...
        totalWeight += coverWeight;
        console.log(
          `  Cover Letter: ${scoreData.cover_letter_score} × ${coverWeight} = ${coverContribution}`
        );
      }
      ...
      if (hasLinkedIn) {
        ...
        totalWeight += 0.1;
        console.log(
          `  LinkedIn: ${scoreData.linkedin_score} × 0.1 = ${linkedinContribution}`
        );
      }
      ...
      if (totalWeight === 0) {
        ...
        console.log(
          `  Weighted calculation: ${
            (overallScore / totalWeight) * totalWeight
          } / ${totalWeight} = ${rawScore.toFixed(2)}, Final: ${overallScore}`
        );
      }
      ...
      if (materials.resume) {
        console.log(`🔍 [QUALITY SCORING] Analyzing resume structure:`, {
          ...
          sectionsKeys: materials.resume.sections
            ? Object.keys(materials.resume.sections)
            : [],
        ...
        const experienceSection =
          materials.resume.sections?.experience ||
          materials.resume.sections?.employment;
          ...
          if (Array.isArray(experienceSection)) {
            ...
            experienceSection.forEach((exp, idx) => {
              if (exp && typeof exp === "object") {
                ...
                if (exp.description) {
                  const bullets =
                    typeof exp.description === "string"
                      ? exp.description
                          .split(/\n|•|\r/)
                          .filter((b) => b.trim() && b.trim().length > 0)
                      : Array.isArray(exp.description)
                      ? exp.description.filter((b) => b && b.trim().length > 0)
                      : [];
                  totalBullets += bullets.length;
                  console.log(
                    `  Experience ${idx + 1}: ${
                      exp.title || exp.position || "Untitled"
                    } at ${exp.company || "Unknown"} - ${
                      bullets.length
                    } bullets`
                  );
                }
                ...
                if (exp.bullets && Array.isArray(exp.bullets)) {
                  const bulletCount = exp.bullets.filter(
                    (b) => b && b.trim().length > 0
                  ).length;
                  ...
                  console.log(
                    `  Experience ${
                      idx + 1
                    }: Additional ${bulletCount} bullets from bullets array`
                  );
                }
                // Check achievements array field
                if (exp.achievements && Array.isArray(exp.achievements)) {
                  const achievementCount = exp.achievements.filter(
                    (b) => b && b.trim().length > 0
                  ).length;
                  ...
                  console.log(
                    `  Experience ${
                      idx + 1
                    }: Additional ${achievementCount} bullets from achievements array`
                  );
                }
              }
            });
        ...
        if (experienceCount === 0 && materials.resume.text) {
          ...
          const textExperiences = extractExperiencesFromText(
        ...
        console.log(
          `📊 [QUALITY SCORING] Parsed resume: ${skillCount} skills, ${experienceCount} experiences, ${totalBullets} bullets, ${projectCount} projects`
        );
      }
      ...
      console.log(
        `📊 [QUALITY SCORING] Resume analysis: ${skillCount} skills, ${experienceCount} experiences, ${totalBullets} bullets, ${projectCount} projects`
      );
      console.log(
        `📊 [QUALITY SCORING] Job requires: ${requiredSkillsCount} skills`
      );
      if (experienceCount > 0) {
        console.log(
          `📊 [QUALITY SCORING] Bullet analysis: ${totalBullets} total bullets ÷ ${experienceCount} jobs = ${(
            totalBullets / experienceCount
          ).toFixed(
            1
          )} bullets per job (industry standard: 4-6 bullets per job is good)`
        );
      }
      ...
      if (skillCount <= 5) {
        ...
        console.log(
          `⚠️ [QUALITY SCORING] Applying penalty: Resume has only ${skillCount} skills, capping score at 60`
        );
      } else if (skillCount < 8) {
        adjustedScore = Math.min(adjustedScore, 70); // Was 65, now 70
        console.log(
          `⚠️ [QUALITY SCORING] Applying penalty: Resume has only ${skillCount} skills, capping score at 70`
        );
      }
      ...
      if (experienceCount === 1) {
        ...
        console.log(
          `⚠️ [QUALITY SCORING] Applying penalty: Resume has only 1 experience, reducing score by 15`
        );
      } else if (experienceCount === 2) {
        adjustedScore = Math.max(0, adjustedScore - 5); // Was 10, now 5
        console.log(
          `⚠️ [QUALITY SCORING] Applying penalty: Resume has only 2 experiences, reducing score by 5`
        );
      }
      ...
      if (experienceCount > 0) {
        ...
        console.log(
          `✅ [QUALITY SCORING] Bullet count: ${totalBullets} bullets for ${experienceCount} jobs (avg ${avgBulletsPerJob.toFixed(
            1
          )} per job) - no penalty applied`
        );
      }
      ...
      if (requiredSkillsCount > 0 && skillCount > 0) {
        ...
        if (matchPercentage < 50) {
          ...
          console.log(
            `⚠️ [QUALITY SCORING] Applying penalty: Resume has only ${matchPercentage.toFixed(
              0
            )}% of required skills, capping score at 65`
          );
          ...
          console.log(
            `⚠️ [QUALITY SCORING] Applying penalty: Resume has only ${matchPercentage.toFixed(
              0
            )}% of required skills, capping score at 75`
          );
        }
      }
      ...
      if (
        ...
        console.log(
          `✅ [QUALITY SCORING] Applying reward: Comprehensive resume (${skillCount} skills, ${experienceCount} jobs, ${totalBullets} bullets, ${projectCount} projects), adding 15 points`
        );
      } else if (
        skillCount >= 10 &&
        experienceCount >= 2 &&
        ...
        console.log(
          `✅ [QUALITY SCORING] Applying reward: Good resume (${skillCount} skills, ${experienceCount} jobs, ${totalBullets} bullets, ${projectCount} projects), adding 10 points`
        );
      } else if (projectCount >= 2) {
        adjustedScore = Math.min(100, adjustedScore + 5);
        console.log(
          `✅ [QUALITY SCORING] Applying reward: Resume has 2+ projects, adding 5 points`
        );
      }
      ...
      if (
        ...
        materials.job &&
        materials.job.required_skills &&
        Array.isArray(materials.job.required_skills) &&
        materials.job.required_skills.length > 0
      ) {
        const resumeSkills = extractSkillsFromResume(materials.resume);
        const requiredSkills = materials.job.required_skills.map((s) =>
          s.toLowerCase().trim()
        );
        ...
        const matchedSkills = requiredSkills.filter((reqSkill) => {
          ...
          if (
            normalizedResumeSkills.some(
              (resSkill) => normalizeSkill(resSkill) === reqNormalized
            )
          ) {
            return true;
          }
        ...
        if (
          ...
          scoreData.score_breakdown.skills_alignment !== null &&
          scoreData.score_breakdown.skills_alignment !== undefined
        ) {
          ...
          console.log(
            `✅ [QUALITY SCORING] Skills alignment bonus: ${
              matchedSkills.length
            } matched skills (${matchedSkills.slice(0, 5).join(", ")}${
              matchedSkills.length > 5 ? "..." : ""
            }), adding ${bonus} points. Score: ${currentScore} → ${newScore}`
          );
        }
      }
      ...
      console.log(
        `✅ [QUALITY SCORING] Final overall score: ${overallScore} (calculated from component scores with post-processing adjustments)`
      );
      ...
      console.log(
        `✅ [QUALITY SCORING] Analysis complete. Score: ${overallScore}/100`
      );
