...
describe("Employment Routes", () => {
  ...
  describe("GET /api/employment", () => {
    ...
    it("should return 401 when not authenticated", async () => {
      const response = await request(app).get("/api/employment");
  ...
  describe("DELETE /api/employment/:id - Error Cases", () => {
    it("should return 404 for non-existent employment entry", async () => {
      const response = await request(app)
        .delete("/api/employment/999999")
        .set("Authorization", `Bearer ${user.token}`);
    ...
    it("should reject delete for other user's employment", async () => {
      const otherUser = await createTestUser();
      const otherEmployments = await seedEmployment(otherUser.id, 1);
      const otherEmploymentId = otherEmployments[0].id;

      const response = await request(app)
        .delete(`/api/employment/${otherEmploymentId}`)
        .set("Authorization", `Bearer ${user.token}`);
  ...
  describe("PUT /api/employment/:id - Error Cases", () => {
    it("should return 404 for non-existent employment entry", async () => {
      const updateData = {
        title: "Senior Software Engineer",
        company: "Updated Company",
    ...
    it("should return 400 for missing required fields", async () => {
      ...
      const response = await request(app)
        .put(`/api/employment/${employmentId}`)
        .set("Authorization", `Bearer ${user.token}`)
        .send({ location: "New Location" }); // Missing title, company, start_date
