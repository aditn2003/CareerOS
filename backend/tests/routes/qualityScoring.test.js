...
describe("Quality Scoring Routes", () => {
  ...
  describe("POST /api/quality-scoring/:jobId/analyze", () => {
    it("should analyze application quality", async () => {
      ...
      pool.query.mockImplementation((query) => {
        ...
        if (
          query.includes(
            "SELECT id, overall_score FROM application_quality_scores"
          )
        ) {
          return Promise.resolve({ rows: [] });
        }
      ...
      if (global.__qualityScoringMockService) {
        global.__qualityScoringMockService.analyzeApplicationQuality.mockResolvedValue(
          mockQualityScore
        );
      }
    ...
    it("should return cached score if recent", async () => {
      pool.query.mockImplementation((query) => {
        ...
        if (
          query.includes("SELECT id, overall_score, created_at, updated_at") &&
  ...
  describe("GET /api/quality-scoring/:jobId/history", () => {
    it("should get score history for a job", async () => {
      pool.query.mockImplementation((query) => {
        ...
        if (
          query.includes(
            "SELECT id, overall_score FROM application_quality_score_history"
          )
        ) {
          return Promise.resolve({
            rows: [
  ...
  describe("PUT /api/quality-scoring/user/threshold", () => {
    ...
    it("should handle database error", async () => {
      ...
      expect(response.body.error).toBe("Failed to update threshold");
    });
  });

  describe("Error Handling", () => {
    it("should handle database error in POST analyze", async () => {
      ...
      expect(response.status).toBe(500);
      expect(response.body.error).toBe("Failed to analyze application quality");
    });

    it("should handle database error in GET score", async () => {
      pool.query.mockRejectedValueOnce(new Error("Database connection failed"));
      ...
      expect(response.status).toBe(500);
      expect(response.body.error).toBe("Failed to fetch quality score");
    });

    it("should return 404 if job not found in GET score", async () => {
      pool.query.mockResolvedValueOnce({ rows: [] }); // Job check returns empty

      const response = await request(app)
        .get("/api/quality-scoring/999")
        .set("Authorization", `Bearer ${user.token}`);
    ...
    it("should handle database error in GET user stats", async () => {
      pool.query.mockRejectedValueOnce(new Error("Database connection failed"));
      ...
      expect(response.status).toBe(500);
      expect(response.body.error).toBe("Failed to fetch user statistics");
    });
    ...
    it("should handle database error in GET history", async () => {
      ...
      expect(response.status).toBe(500);
      expect(response.body.error).toBe("Failed to fetch score history");
    });

    it("should handle service error in analyze", async () => {
      pool.query.mockImplementation((query) => {
        if (query.includes("SELECT id, title, company FROM jobs")) {
          return Promise.resolve({
            rows: [{ id: 1, title: "Software Engineer", company: "Tech Corp" }],
        ...
        if (query.includes("SELECT id, overall_score")) {
          return Promise.resolve({ rows: [] });
        }
        if (query.includes("SELECT AVG(overall_score)")) {
          return Promise.resolve({
            rows: [{ average_score: null, top_score: null }],
          });
        }
        return Promise.resolve({ rows: [] });
      });
      ...
      if (global.__qualityScoringMockService) {
        global.__qualityScoringMockService.analyzeApplicationQuality.mockRejectedValue(
    ...
    it("should return 401 if not authenticated", async () => {
      const response = await request(app).get("/api/quality-scoring/1");
